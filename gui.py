...
INSTRUCTIONS = [
	...
	"FCOMI", "FCOMIP", "FUCOMI", "FUCOMIP", "fxrstor", "fxsave", "fisttp", "emms", "movd", "movq", "jc"]

INSTRUCTIONS.sort(key=len, reverse=True)

CLEFS = [
	"section", "%macro", "%endmacro", "equ", "$", "$$", "times", "%include", "org", "byte", "word", "dword", "qword"]
...
class ColorParser():
	...
	def parse(self):
		...
		for ligne in range(len(text)):
			...
			if commentaire >= 0:
				rep = ("commentaire", f"{str(ligne+1)}.{str(commentaire)}", f"{str(ligne+1)}.{str(len(text[ligne]))}")
				self.colors.append(rep)
